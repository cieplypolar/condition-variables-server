 ...
 */
  ...
  /* Tell VFS and CVS about the exiting process. */
  m.m_type = dump_core ? PM_DUMPCORE : PM_EXIT;
  m.PM_PROC = rmp->mp_endpoint;
  m.PM_TRACED_PROC = rmp->mp_endpoint;
  m_to_cvs.m_type = dump_core ? PM_DUMPCORE : PM_EXIT;
  m_to_cvs.PM_PROC = rmp->mp_endpoint;
  m_to_cvs.PM_TRACED_PROC = rmp->mp_endpoint;

  if (dump_core) {
    m.PM_TERM_SIG = rmp->mp_sigstatus;
    m.PM_PATH = rmp->mp_name;
    m_to_cvs.PM_TERM_SIG = rmp->mp_sigstatus;
    m_to_cvs.PM_PATH = rmp->mp_name;
  }
